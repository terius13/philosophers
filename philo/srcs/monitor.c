...
/*   Updated: 2024/05/07 21:00:59 by ting             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/philo.h"

// loop thru all philos to check their last meal against time_to_die
int	philos_are_dead(t_table *table)
{
	...
	while (i < table->num_of_philos)
	{
		...
		if ((get_time() - philo[i].last_meal) > table->time_to_die
			&& philo[i].meal_count != table->num_of_meals)
			dead_flag = 1;
...
// check if all philos have ate, if yes, end simulation set to 1
int	philos_all_ate(t_table *table)
{
	...
	{
		pthread_mutex_lock(&table->meal_lock);
		if (philo[i].meal_count >= table->num_of_meals
